...
export function SubAtomicParticleField({
  ...
  const round = useCallback((value: number) => Math.round(value * 1_000) / 1_000, []);

  useEffect(() => {
    if (typeof navigator === 'undefined') return;
    setIsLowEndDevice((navigator.hardwareConcurrency || 4) < 4);
  }, []);
  ...
  const particles = useMemo<Particle[]>(() => {
    ...
    return Array.from({ length: count }, (_, index) => {
      ...
      const orbit = round(orbitBase + orbitJitter);
      ...
      const finalDuration = round(duration * curiosityVariance);
      const delay = round((index * 0.35 + bondVal / 90) % Math.max(finalDuration, 0.0001));

      return {
        id: index,
        orbit,
        size: round(1.4 + sizePulse),
        duration: finalDuration,
        delay,
        twinkle: round(1.2 + (index % 4) * 0.3),
        twinkleDelay: round((index * 0.15) % 2),
        opacity: round((0.35 + (bondInfluence * (index % 7)) / 25) * intensityMod),
      };
    });
  }, [bond, curiosity, energy, consciousParticleCount, consciousParticleSpeed, consciousParticleSize, consciousColorIntensity, flowPattern, isLowEndDevice, round]);
      ...
      {particles.map((particle) => (
        <g
          ...
          style={{
            ...
            animationIterationCount: reduceMotion ? 1 : 'infinite',
