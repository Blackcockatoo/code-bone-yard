 ...
 * @param traits Optional derived traits for genome-based metabolism. If not provided, defaults are used.
...
export function deterministicVitalsTick(
  ...
  while (newVitals.tickAccumulator >= SIMULATION_TICK_MS) {
    ...
    // Derive parameters from genome traits
    const metabolismRate = traits
      ? 0.8 +
        traits.physical.size * 0.3 +
        (traits.personality.energy / 50) * 0.2
      : 1.0;
    const baseHealthRegen = traits
      ? 0.3 +
        traits.physical.size * 0.3 +
        (traits.personality.discipline - 50) / 100
      : 0.5;
    const moodSensitivity = traits
      ? 3.0 - traits.personality.discipline / 50
      : 2.0;
