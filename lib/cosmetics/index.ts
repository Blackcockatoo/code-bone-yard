...
export const COSMETICS_CATALOG: Cosmetic[] = [
  // Accessories
  {
    ...
    visualData: {
      color: "#FFD700",
      svgPath: "M10,5 L15,10 L20,5 L18,15 L12,15 Z",
    },
  },
  {
    id: "halo-sacred",
 ...
 */
export function checkUnlockConditions(gameState: {
  ...
  COSMETICS_CATALOG.forEach((cosmetic) => {
    ...
    if (
      condition.includes("speciation") &&
      gameState.evolution.state === "SPECIATION"
    ) {
    ...
    if (
      condition.includes("100 samples") &&
      gameState.vimana.totalSamples >= 100
    ) {
      unlocked.push(cosmetic.id);
    }
    if (
      condition.includes("5 offspring") &&
      gameState.breeding.offspringCount >= 5
    ) {
      unlocked.push(cosmetic.id);
    }
    if (
      condition.includes("all vimana") &&
      gameState.vimana.anomaliesResolved >= 16
    ) {
      unlocked.push(cosmetic.id);
    }
    if (
      condition.includes("20 mini-games") &&
      gameState.miniGames.totalPlays >= 20
    ) {
      unlocked.push(cosmetic.id);
    }
    if (
      condition.includes("quantum") &&
      gameState.evolution.state === "QUANTUM"
    ) {
      unlocked.push(cosmetic.id);
    }
  });
