...
import type { DreamJournalState } from "@metapet/core/dreamjournal/types";
import { createDefaultDreamJournal } from "@metapet/core/dreamjournal/types";
...
export interface PetSaveData {
  ...
  vimana: VimanaState;
  dreamJournal?: DreamJournalState;
  crest: PrimeTailId;
...
/**
 * Retry wrapper for transient IndexedDB failures
 */
...
/**
 * Save pet data (internal implementation)
 */
async function _savePetInternal(data: PetSaveData): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    const saveData = {
      ...
      dreamJournal: data.dreamJournal
        ? {
            ...data.dreamJournal,
            dreams: data.dreamJournal.dreams.map((d) => ({ ...d })),
            archetypeCount: { ...data.dreamJournal.archetypeCount },
          }
        : undefined,
...
/**
 * Save pet data with retry logic
 */
export async function savePet(data: PetSaveData): Promise<void> {
  return withRetry(() => _savePetInternal(data));
}

/**
 * Load pet data by ID (internal implementation)
 */
async function _loadPetInternal(id: string): Promise<PetSaveData | null> {
  const db = await initDB();
...
/**
 * Load pet data by ID with retry logic
 */
export async function loadPet(id: string): Promise<PetSaveData | null> {
  return withRetry(() => _loadPetInternal(id));
}

/**
 * Get all pets (internal implementation)
 */
async function _getAllPetsInternal(): Promise<PetSaveData[]> {
  const db = await initDB();
...
/**
 * Get all pets with retry logic
 */
export async function getAllPets(): Promise<PetSaveData[]> {
  return withRetry(() => _getAllPetsInternal());
}
...
export function importPetFromJSON(
  ...
  const genome: Genome =
    options?.skipGenomeValidation && !isValidGenome(parsed.genome)
      ? {
...
function normalizePetData(raw: unknown): PetSaveData {
  ...
  const dreamJournal = isValidDreamJournal(typed.dreamJournal)
    ? {
        ...typed.dreamJournal,
        dreams: typed.dreamJournal.dreams.map((d) => ({ ...d })),
        archetypeCount: { ...typed.dreamJournal.archetypeCount },
      }
    : createDefaultDreamJournal();
...
function isValidDreamJournal(value: unknown): value is DreamJournalState {
  if (!value || typeof value !== "object") return false;
  const journal = value as DreamJournalState;

  // Check basic structure
  if (!Array.isArray(journal.dreams)) return false;
  if (typeof journal.totalDreams !== "number") return false;
  if (!journal.archetypeCount || typeof journal.archetypeCount !== "object")
    return false;

  // Validate activeDreamResidue if present
  if (journal.activeDreamResidue !== null) {
    if (typeof journal.activeDreamResidue !== "object") return false;
    const residue = journal.activeDreamResidue;
    if (
      typeof residue.name !== "string" ||
      typeof residue.description !== "string" ||
      typeof residue.value !== "number" ||
      typeof residue.appliedAt !== "number" ||
      typeof residue.expiresAt !== "number"
    ) {
      return false;
    }
  }
