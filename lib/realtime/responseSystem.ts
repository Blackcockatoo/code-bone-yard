...
export function getResponse(
  ...
  const moodLevel =
    context.mood > 70 ? "happy" : context.mood > 40 ? "neutral" : "unhappy";
  ...
  switch (action) {
    case "feed":
      responses =
        responseLibrary.feeding[isVeryTired ? "neutral" : moodLevel] ||
        responseLibrary.feeding.neutral;
      responseType = "action";
      duration = 2500;
    ...
    case "play":
      responses =
        responseLibrary.playing[isVeryTired ? "tired" : moodLevel] ||
        responseLibrary.playing.neutral;
    ...
    case "clean":
      responses =
        responseLibrary.cleaning[moodLevel] || responseLibrary.cleaning.neutral;
      responseType = "action";
      duration = 2500;
      audioTrigger = "success";
      break;
  ...
  const response: PetResponse = {
    ...
    intensity:
      (selected.intensity as "subtle" | "normal" | "intense") || "normal",
    duration,
    hapticFeedback:
      selected.intensity === "intense"
        ? "heavy"
        : selected.intensity === "normal"
          ? "medium"
          : "light",
    audioTrigger,
    chainReaction,
 ...
 */
export function getWarningResponse(
  context: ResponseContext,
): PetResponse | null {
 ...
 */
export function getAnticipatoryResponse(
  context: ResponseContext,
): PetResponse | null {
  const avgVitals =
    (context.mood + context.energy + (100 - context.hunger) + context.hygiene) /
    4;
