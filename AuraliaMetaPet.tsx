...
import { breed, calculateResonance } from '@/auralia/breeding';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gray-900/80 rounded-2xl p-6 border border-yellow-600/20">
              ...
              <div className="space-y-4">
                <button
                  onClick={() => setShowBreedingCenter(true)}
                  className="w-full px-4 py-3 bg-gradient-to-r from-pink-600 to-purple-600 hover:from-pink-500 hover:to-purple-500 rounded-lg font-medium transition-all flex items-center justify-center gap-2"
                  aria-label="Open Breeding Center"
                >
                  <img src="/images/icon_play.webp" alt="Breeding" className="w-5 h-5 brightness-200" />
                  💞 Enter Breeding Center
                </button>
                
                {offspring.length > 0 && (
                  <div className="mt-4 space-y-2">
                    <h4 className="text-sm font-semibold text-purple-400">Lineage ({offspring.length})</h4>
                    <div className="max-h-48 overflow-y-auto space-y-2">
                      {offspring.map((child, i) => (
                        <div key={i} className="p-3 bg-purple-900/20 border border-purple-500/30 rounded-lg">
                          ...
                          <button
                            onClick={() => switchToOffspring(child)}
                            className="mt-3 w-full py-1 bg-purple-600/40 hover:bg-purple-600/60 rounded text-[10px] uppercase tracking-wider transition-colors"
                          >
                            Switch to this Guardian
                          </button>
                        </div>
                      ))}
                    </div>
      ...
      {showBreedingCenter && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="relative w-full max-w-2xl">
            <button
              onClick={() => setShowBreedingCenter(false)}
              className="absolute -top-12 right-0 text-white/70 hover:text-white text-sm flex items-center gap-1"
            >
              ✕ Close
            </button>
            <BreedingCenter
